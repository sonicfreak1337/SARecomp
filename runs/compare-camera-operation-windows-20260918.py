...
a=json.loads((base/'camera-operation-gamma-win-off-20260918/result.json').read_text())
b=json.loads((base/'camera-operation-gamma-win-on-20260918/result.json').read_text())
ends={}
for f in (5,25):
...
for label,data in [('off',a),('on',b)]:
 ...
 counts[label]={k:int(y[k])-int(x[k]) for k in y if any(w in k for w in ('render_hierarchy','render_model','contact_family','model_pipeline','land_render','object_contact','camera_operation')) and y[k].isdigit() and x.get(k,'').isdigit()}
paths=[base/f'camera-operation-gamma-win-{v}-20260918/frames/frame-270.bmp' for v in ('off','on')]
...
r['group_configuration']={label:{k:data[k] for k in ('native_movement_contact','native_model_submission','native_land_render','native_object_contact','native_camera_operation')} for label,data in [('off',a),('on',b)]}
r['matched_work']=not any(ends.values()) and all(r['completed']) and a['frame_window_valid'] and b['frame_window_valid']
(base/'camera-operation-gameplay-comparison-windows-20260918.json').write_text(json.dumps(r,indent=2)+'\n')
