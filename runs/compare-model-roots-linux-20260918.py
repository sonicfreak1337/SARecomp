...
for label,data in [('off',a),('on',b)]:
 ...
 counters[label]={k:int(y[k])-int(x[k]) for k in y if any(w in k for w in ('render_hierarchy','render_model','rigid_hierarchy','morph_hierarchy','inverse_trig','inverse_memory','collision_world','contact_family','model_pipeline')) and y[k].isdigit() and x.get(k,'').isdigit()}
r={'schema':'sarecomp-native-model-roots-contact-comparison-v1','scenario':a['scenario'],'exe_sha256':[a['exe_sha256'],b['exe_sha256']],'fields':fields,'off':a['measurement'],'on':b['measurement'],'endpoints':ends,'configuration_differences':config,'group_configuration':{label:{k:d.get(k) for k in ('native_movement_contact','native_model_submission')} for label,d in [('off',a),('on',b)]},'percent':{k:100*(b['measurement'][k]/a['measurement'][k]-1) for k in metrics},'window_counters':counters,'matched_work':a['exe_sha256']==b['exe_sha256'] and not a.get('forced_stop',False) and not b.get('forced_stop',False) and not any(ends.values()) and not config and a['completed'] and b['completed'] and a['frame_window_valid'] and b['frame_window_valid'] and a['measurement']['game_updates']==b['measurement']['game_updates']}
