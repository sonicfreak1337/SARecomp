...
int main(int argc,char** argv){
    try{
        if(argc!=3&&argc!=4)throw std::runtime_error("renderer, cache directory, optional vsync mode required");
        ...
        settings.presentation_fps=settings.vsync==1?30:60;
        const auto ini=cache/L"sonic-display.ini";sonic::presentation::save_settings(ini,settings);
        SetEnvironmentVariableW(L"SARECOMP_DISPLAY_CONFIG",ini.c_str());sonic::presentation::initialize(cache/L"game.exe");
        NativePortGraphicsConfig graphics;graphics.output_extent={640,480};graphics.render_extent={320,240};graphics.initially_visible=false;graphics.synchronize_present=false;
        ...
        for(unsigned rate: settings.vsync==1 ? std::initializer_list<unsigned>{30u,144u} : std::initializer_list<unsigned>{60u,120u,144u}){
            // Exercise the same live FPS-setting path as Options, retaining
            // the window/device and timer ownership across cap changes.
            settings.presentation_fps=rate;sonic::presentation::apply_live(settings);
            ...
            for(unsigned frame=0;frame<=78;++frame){
                host.wait_until_title_deadline(epoch+std::uint64_t(frame)*1'000'000'000/30);
            ...
            std::cout<<"SONIC_PACING_SAMPLE renderer="<<argv[1]<<" target="<<rate<<" output_fps="<<output_fps
            ...
            if(simulation_fps<27||simulation_fps>33||output_fps<20||
                (settings.vsync!=1&&(output_fps<rate*.85||output_fps>rate*1.15))||
                (output_fps>33&&s.repeated_presentations<=repeated))
                throw std::runtime_error("presentation rate changed title cadence or failed to repeat completed frames");
        }
        std::cout<<"SONIC_PACING_OK renderer="<<argv[1]<<" vsync="<<settings.vsync<<" simulation=unchanged\n";
        return 0;
